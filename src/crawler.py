...
HEADERS = {
    ...
    "Accept": "text/html,applicationhttps://readmanga.live/list/genres/sort_name/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8",
...
def crawl_manga(genre, name, relative_url):
    ...
    try:
        chapter_page.html.render(timeout=50, scrolldown=10000)
    except Exception as e:
        with open("./crawler_logs.txt", "a+") as f:
            f.write(f"ERROR: crawl_manga failed to render {name} in {genre}: {e}")
        return
...
def crawl(relative_path: str):
    ...
    dom = BeautifulSoup(page, 'html.parser', from_encoding='utf-8')
